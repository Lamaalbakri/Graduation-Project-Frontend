...
import {
  updateDistributorsGoodsRequestStatus,
  moveDistributorsGoodsCurrentToPrevious,
} from "../../../api/distributerGoodsRequestsAPI";
import { DataGrid } from "@mui/x-data-grid";
import "./RequestsTableForDistributedGoods.css";

function RequestsTableForDistributedGoods({ data }) {
  ...
  const [requests, setRequests] = useState(
    data.map((request) => ({
      ...
      statusClass: `ManageGoodsDistributors-status-${request.status}`, //Add a new property to the object, statusClass to style the drop-down list status.
      receiverId: request.retailerId,
      receiverType: request.receiverType || "retailer",
    }))
  );
  ...
  const updateRequestStatus = async (id, newStatus) => {
    try {
      ...
      const updatedRequest = await updateDistributorsGoodsRequestStatus(
        id,
        newStatus
      );
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      const moveResult = await moveDistributorsGoodsCurrentToPrevious(
        selectedRequestId
      );
  ...
  const columns = [
    ...
    {
      field: "goodsForRetailers",
      headerName: "Goods",
      width: 140,
      headerAlign: "left",
      renderCell: (params) => (
        <div className="ManageGoodsDistributors-cell-content">
    ...
    },
    {
      ...
      headerAlign: "left",
      renderCell: (params) => {
        const statusClass = `ManageGoodsDistributors-status-${params.row.status}`;
        ...
        if (
          ...
          return (
            <div
              className={`ManageGoodsDistributors-status-text-no-drop-${params.row.status}`}
            >
              {formattedStatus}
            </div>
          );
        ...
        return (
          <select
            ...
            className={`ManageGoodsDistributors-status-select ${statusClass}`}
            onChange={(e) =>
    ...
    },
    {
      ...
      headerAlign: "left",
      renderCell: (params) => (
        <div className="ManageGoodsDistributors-contract-button">
    ...
    },
    {
      ...
      headerAlign: "left",
      renderCell: (params) => (
        <div className="ManageGoodsDistributors-action-buttons">
          <button
            className="ManageGoodsDistributors-tracking-icon"
            onClick={() =>
              toggleDialog(
                "trackingDialog",
                true,
